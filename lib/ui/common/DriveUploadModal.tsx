            ...
            <div className="grid gap-6">
              ...
              <div>
                ...
                {files.length === 0 ? (
                  <div className="rounded-2xl border-2 border-dashed border-neutral-200 py-10 flex flex-col items-center justify-center text-neutral-400">
                    ...
                    <p className="mt-2 text-sm">아직 올라온 파일이 없어요.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-3 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
                    {files.map(file => (
                      <div key={file.id} className="group relative rounded-xl border border-neutral-100 bg-neutral-50 p-2 overflow-hidden">
                        <div className="aspect-video rounded-lg bg-neutral-200 flex items-center justify-center text-[10px] text-neutral-400 font-bold overflow-hidden">
                          {file.mimeType.startsWith('image/') ? (
                            <img src={file.webViewLink?.replace('view', 'thumbnail')} alt="" className="w-full h-full object-cover" />
                          ) : (
                            <span>{file.name.split('.').pop()?.toUpperCase() || 'FILE'}</span>
                          )}
                        </div>
                        <div className="mt-2 text-[11px] truncate font-medium text-neutral-700">{file.name}</div>
                        <button 
                          onClick={() => handleDelete(file.id)}
                          className="absolute top-1 right-1 p-1 bg-white/80 hover:bg-white rounded-full shadow-sm text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  ...
                  disabled={uploading}
                  className="flex flex-col items-center justify-center py-4 rounded-2xl border border-neutral-200 hover:bg-neutral-50 transition-colors disabled:opacity-50"
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-black"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>
                  <span className="mt-2 text-sm font-bold">사진 찍기</span>
                  <input 
                    type="file" 
                    accept="image/*" 
                    capture="environment" 
                ...
                <button 
                  ...
                  className="flex flex-col items-center justify-center py-4 rounded-2xl border border-neutral-200 hover:bg-neutral-50 transition-colors disabled:opacity-50"
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-black"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path><polyline points="13 2 13 9 20 9"></polyline></svg>
